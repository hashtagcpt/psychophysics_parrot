...
if params(1)==1                                                         % zero padded image
    ...
    imageAmp=abs(fft2(targIm, imSize(1)*2, imSize(2)*2));  
    % fft of image, padded with zeros to edge
    size(log(radDist(2:end)))
    size(log(imageAmp(2:end)))
...
elseif params(1)==2                                                     % average all amplitudes within each octave
	...
	[X,Y]=meshgrid(-imRadius(1):-imRadius(1)+imSize(1)-1,-imRadius(2):-imRadius(2)+imSize(2)-1);          % 2D matrix of radial distances from centre
	radDist=(X.^2+Y.^2).^0.5;                                           % radial distance from centre
    radDist=fftshift(radDist);                                          % shift zero cpi to (1,1)
